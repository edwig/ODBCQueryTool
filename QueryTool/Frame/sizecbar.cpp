...
// 02.06.2002 changed by Kochetov for flicker reduction - result is not so good
    ...
    else
    {
        ...
        for (int i = nFirst; nDelta != 0 && i != nLimit; i += (bBefore ? -1 : 1))
        {
            ...
            int nDeltaT = min(nDelta, 
                (int)((bHorz ? pBar->m_szHorz.cx : pBar->m_szVert.cy) -
    ...
    for (i = nFirst; i <= nLast; i++)
    {
        ...
        if (bIsSizingBar)
        {
            nLengthActual += bHorz ? pBar->m_szHorz.cx - 2 : 
                pBar->m_szVert.cy - 2;
            nWidthMax = max(nWidthMax, (int)(bHorz ? pBar->m_szHorz.cy :
                pBar->m_szVert.cx));
        }
        else
...
BOOL CSCBMiniDockFrameWnd::Create(CWnd* pParent, DWORD dwBarStyle)
{
    ...
    if (!CMiniFrameWnd::CreateEx(dwExStyle,
        NULL, "ddd"/*&afxChNil*/, dwStyle, rectDefault, pParent))
    {
