...
class MCGridCell : public MCGridCellBase
{
    friend class MCGridCtrl;
    DECLARE_DYNCREATE(MCGridCell)
...
// Attributes
public:
    void operator=(const MCGridCell& cell);
    ...
    virtual void  SetGrid(MCGridCtrl* pGrid)     { m_pGrid   = pGrid;   }
    ...
    virtual void  SetSQLType(int sqlType)        { m_sqlType = sqlType; }
    ...
    virtual MCGridCtrl* GetGrid() const             { return m_pGrid;   }
    // virtual DWORD    GetState() const - use base class
    virtual DWORD       GetFormat() const;
    virtual int         GetSQLType() const          { return m_sqlType; }
...
protected:
    ...
    DWORD       m_nFormat;
    int         m_sqlType;      // Type in SQL database
    COLORREF    m_crFgClr;
    COLORREF    m_crBkClr;
    LOGFONT*    m_plfFont;
    UINT        m_nMargin;
    ...
    MCGridCtrl* m_pGrid;        // Parent grid control
    CWnd*       m_pEditWnd;
};

// This class is for storing grid default values. It's a little heavy weight, so
// don't use it in bulk 
class MCGridDefaultCell : public MCGridCell
{
    DECLARE_DYNCREATE(MCGridDefaultCell)

// Construction/Destruction
public:
    MCGridDefaultCell();
    virtual ~MCGridDefaultCell();
...
public:
    ...
    virtual DWORD    GetState() const                       { return MCGridCell::GetState()|GVIS_READONLY; }
