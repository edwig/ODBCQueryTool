...
IMPLEMENT_DYNAMIC(MCGridCellBase, CObject)

/////////////////////////////////////////////////////////////////////////////
// GridCellBase

MCGridCellBase::MCGridCellBase()
{
    Reset();
}
...
void MCGridCellBase::Reset()
{
    m_nState  = 0;
}

void MCGridCellBase::operator=(const MCGridCellBase& cell)
{
	if (this == &cell) return;
...
MCGridCellBase* MCGridCellBase::GetDefaultCell() const
{
    if (GetGrid())
...
// of bounds.
BOOL MCGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    {
        MCCellID FocusCell = pGrid->GetFocusCell();
...
// Not yet implemented
void MCGridCellBase::OnMouseEnter()
{
    TRACE0("Mouse entered cell\n");
}

void MCGridCellBase::OnMouseOver()
{
    //TRACE0("Mouse over cell\n");
}

// Not Yet Implemented
void MCGridCellBase::OnMouseLeave()
{
    TRACE0("Mouse left cell\n");
}

void MCGridCellBase::OnClick( CPoint PointCellRelative)
{
    UNUSED_ALWAYS(PointCellRelative);
    TRACE2("Mouse Left btn up in cell at x=%i y=%i\n", PointCellRelative.x, PointCellRelative.y);
}

void MCGridCellBase::OnClickDown( CPoint PointCellRelative)
{
    UNUSED_ALWAYS(PointCellRelative);
    TRACE2("Mouse Left btn down in cell at x=%i y=%i\n", PointCellRelative.x, PointCellRelative.y);
}

void MCGridCellBase::OnRClick( CPoint PointCellRelative)
{
    UNUSED_ALWAYS(PointCellRelative);
    TRACE2("Mouse right-clicked in cell at x=%i y=%i\n", PointCellRelative.x, PointCellRelative.y);
}

void MCGridCellBase::OnDblClick( CPoint PointCellRelative)
{
    UNUSED_ALWAYS(PointCellRelative);
...
BOOL MCGridCellBase::OnSetCursor()
{
...
void MCGridCellBase::OnEndEdit() 
{
	ASSERT( FALSE); 
}

BOOL MCGridCellBase::ValidateEdit(LPCTSTR str)
{
    UNUSED_ALWAYS(str);
	return TRUE;
}
...
BOOL MCGridCellBase::GetTextRect( LPRECT pRect)  // i/o:  i=dims of cell rect; o=dims of text rect
{
    if (GetImage() >= 0)
    {
        IMAGEINFO Info;
...
CSize MCGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= NULL*/)
{
    MCGridCtrl* pGrid = GetGrid();
    ASSERT(pGrid);
    ...
    if (pDC == NULL || szText == NULL)
    {
        ...
        if (pDC == NULL || szText == NULL) 
        {
            MCGridDefaultCell* pDefCell = (MCGridDefaultCell*) GetDefaultCell();
            ASSERT(pDefCell);
...
CSize MCGridCellBase::GetCellExtent(CDC* pDC)
{
...
// printed correctly.
BOOL MCGridCellBase::PrintCell(CDC* pDC, int /*nRow*/, int /*nCol*/, CRect rect)
{
#if defined(_WIN32_WCE_NO_PRINTING) || defined(GRIDCONTROL_NO_PRINTING)
    return FALSE;
#else
...
*****************************************************************************/
LRESULT MCGridCellBase::SendMessageToParent(int nRow, int nCol, int nMessage)
{
    MCGridCtrl* pGrid = GetGrid();
    if( pGrid)
