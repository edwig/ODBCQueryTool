...
//                            cell text only
//                          - Minor vis bug fixes
//                          - MCGridCtrl now works with CGridCellBase instead of CGridCell
...
#pragma message(" -- MCGridCtrl: No titletips for cells with large data")
#endif
#ifdef GRIDCONTROL_NO_DRAGDROP
#pragma message(" -- MCGridCtrl: No OLE drag and drop")
#endif
#ifdef GRIDCONTROL_NO_CLIPBOARD
#pragma message(" -- MCGridCtrl: No clipboard support")
#endif
#ifdef GRIDCONTROL_NO_PRINTING
#pragma message(" -- MCGridCtrl: No printing support")
#endif
...
MCGridCtrl::MCGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    RegisterWindowClass();
    ...
    m_pRtcDefault = RUNTIME_CLASS(MCGridCell);
...
BOOL MCGridCtrl::RegisterWindowClass()
{
    WNDCLASS wndcls;
...
BOOL MCGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
...
void MCGridCtrl::SetupDefaultCells()
{
...
void MCGridCtrl::PreSubclassWindow()
{
...
LRESULT MCGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
LRESULT MCGridCtrl::SendDisplayRequestToParent(MG_GV_DISPINFO* pDisplayInfo) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
LRESULT MCGridCtrl::SendCacheHintToParent(const MCCellRange& range) const
{
    if (!IsWindow(m_hWnd))
        return 0;

    MG_GV_CACHEHINT CacheHint;
...
BEGIN_MESSAGE_MAP(MCGridCtrl, CWnd)
//EFW - Added ON_WM_RBUTTONUP
//{{AFX_MSG_MAP(MCGridCtrl)
    ON_WM_PAINT()
    ON_WM_HSCROLL()
    ON_WM_VSCROLL()
    ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// MCGridCtrl message handlers

void MCGridCtrl::OnPaint()
{
...
BOOL MCGridCtrl::OnEraseBkgnd(CDC* /*pDC*/)
{
...
void MCGridCtrl::EraseBkgnd(CDC* pDC)
{
...
void MCGridCtrl::OnSize(UINT nType, int cx, int cy)
{  
...
void MCGridCtrl::OnSysColorChange()
{
...
void MCGridCtrl::OnCaptureChanged(CWnd *pWnd)
{
...
void MCGridCtrl::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
...
void MCGridCtrl::OnTimer(UINT_PTR nIDEvent)
{
...
void MCGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
  ...
  {
    MCCellID idOldTopLeft = GetTopleftNonFixedCell();
...
void MCGridCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    CWnd::OnKeyUp(nChar, nRepCnt, nFlags);
}

void MCGridCtrl::OnSysKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void MCGridCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    // EFW - BUG FIX
...
// Added by KiteFly
LRESULT MCGridCtrl::OnImeChar(WPARAM wCharCode, LPARAM)
{
    // EFW - BUG FIX
...
void MCGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
    MG_GV_DISPINFO *pgvDispInfo = (MG_GV_DISPINFO *)pNMHDR;
    MC_ITEM     *pgvItem = &pgvDispInfo->item;
...
void MCGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
    EndEditing();
    ...
    MCCellID idTopLeft = GetTopleftNonFixedCell();
...
void MCGridCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* /*pScrollBar*/)
{
    ...
    case SB_LINEDOWN:
        if (scrollPos < m_nVScrollMax)
        {
            ...
            if (GetScrollPos32(SB_VERT) == scrollPos)
            {
                if(m_view)
                {
                  // On last line, try to read the rest of the query
                  m_view->ReadRestOfQuery();
                  return;
                }
                break;          // didn't work
    ...
    case SB_PAGEDOWN:
        ...
        {
            rect.top = GetFixedRowHeight();
            if(scrollPos + rect.Height() > m_nVScrollMax)
            {
              if(m_view)
              {
                m_view->ReadRestOfQuery();
    ...
    case SB_THUMBTRACK:
        {
          ...
          // Extra read-in?
          if((GetRowCount() - idNewTopLeft.row) * GetFixedRowHeight() < rect.Height())
          {
            if(m_view)
            {
              m_view->ReadRestOfQuery();
...
/////////////////////////////////////////////////////////////////////////////
// MCGridCtrl implementation functions

void MCGridCtrl::OnDraw(CDC* pDC)
{
    if (!m_bAllowDraw)
        return;
    ...
    CRect VisRect;
    MCCellRange VisCellRange = GetVisibleNonFixedCellRange(VisRect);
...
////////////////////////////////////////////////////////////////////////////////////////
// MCGridCtrl Cell selection stuff

// Is a given cell designation valid (ie within the bounds of our number
// of columns/rows)?
BOOL MCGridCtrl::IsValid(int nRow, int nCol) const
{
    return (nRow >= 0 && nRow < m_nRows && nCol >= 0 && nCol < m_nCols);
}

BOOL MCGridCtrl::IsValid(const MCCellID& cell) const
{
...
// of columns/rows)?
BOOL MCGridCtrl::IsValid(const MCCellRange& range) const
{
...
void MCGridCtrl::SetRedraw(BOOL bAllowDraw, BOOL bResetScrollBars /* = FALSE */)
{
...
// or the supplied dc)
BOOL MCGridCtrl::RedrawCell(const MCCellID& cell, CDC* pDC /* = NULL */)
{
    return RedrawCell(cell.row, cell.col, pDC);
}

BOOL MCGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
    BOOL bResult = TRUE;
    ...
    if (pDC)
    {
        ...
        {
            MCGridCellBase* pCell = GetCell(nRow, nCol);
            if (pCell)
...
BOOL MCGridCtrl::RedrawRow(int row)
{
    BOOL bResult = TRUE;
...
BOOL MCGridCtrl::RedrawColumn(int col)
{
    BOOL bResult = TRUE;
...
MCCellID MCGridCtrl::SetFocusCell(int nRow, int nCol)
{
    return SetFocusCell(MCCellID(nRow, nCol));
}

MCCellID MCGridCtrl::SetFocusCell(MCCellID cell)
{
    if (cell == m_idCurrentCell)
        return m_idCurrentCell;

    MCCellID idPrev = m_idCurrentCell;
...
void MCGridCtrl::SetSelectedRange(const MCCellRange& Range,
...
void MCGridCtrl::SetSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
                                 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
    ...
    if (bSelectCells)
    {
        ...
        for (pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
        {
            DWORD key;
            MCCellID cell;
            m_SelectedCellMap.GetNextAssoc(pos, key, (MCCellID&)cell);
            ...
            nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0)
        {
            for (pos = m_PrevSelectedCellMap.GetStartPosition(); pos != NULL; /* nothing */)
            {
                DWORD key;
                MCCellID cell;
                m_PrevSelectedCellMap.GetNextAssoc(pos, key, (MCCellID&)cell);
...
// selects all cells
void MCGridCtrl::SelectAllCells()
{
    if (!m_bEnableSelection)
        return;
...
// selects columns
void MCGridCtrl::SelectColumns(MCCellID currentCell, 
...
// selects rows
void MCGridCtrl::SelectRows(MCCellID currentCell, 
                           BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
...
// selects cells
void MCGridCtrl::SelectCells(MCCellID currentCell, 
                            BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
...
void MCGridCtrl::OnSelecting(const MCCellID& currentCell)
{
    if (!m_bEnableSelection)
        return;
...
void MCGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCTSTR strText)
{
...
void MCGridCtrl::ClearCells(MCCellRange Selection)
{
...
void MCGridCtrl::CutSelectedText()
{
    if (!IsEditable())
        return;
...
DROPEFFECT MCGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState,
                                 CPoint point)
{
...
DROPEFFECT MCGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
                                  CPoint point)
{
    // Any text data available for us?
...
BOOL MCGridCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT /*dropEffect*/,
                       CPoint /* point */)
{
    m_MouseMode = MOUSE_NOTHING;
...
void MCGridCtrl::OnEditPaste()
{
    if (!IsEditable())
        return;

	MCCellRange cellRange = GetSelectedCellRange();
...
void MCGridCtrl::OnEditSelectAll()
{
...
#ifndef GRIDCONTROL_NO_CLIPBOARD
void MCGridCtrl::OnUpdateEditCopy(CCmdUI* pCmdUI)
{
...
void MCGridCtrl::OnUpdateEditCut(CCmdUI* pCmdUI)
{
...
void MCGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI)
{
...
void MCGridCtrl::OnUpdateEditSelectAll(CCmdUI* pCmdUI)
{
...
BOOL MCGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
    if (point.x >= GetFixedColumnWidth())
        return FALSE;

    MCCellID idCurrentCell = GetCellFromPt(point);
    CPoint start;
...
BOOL MCGridCtrl::MouseOverColumnResizeArea(CPoint& point)
{
...
MCCellID MCGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
    MCCellID cellID; // return value
...
////////////////////////////////////////////////////////////////////////////////
// MCGridCtrl cellrange functions

// Gets the first non-fixed cell ID
MCCellID MCGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
...
MCCellRange MCGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/, 
                                                  BOOL bForceRecalculation /*=FALSE*/)
{
    CRect rect;
    GetClientRect(rect);

    MCCellID idTopLeft = GetTopleftNonFixedCell(bForceRecalculation);
    ...
    return MCCellRange(idTopLeft.row, idTopLeft.col, maxVisibleRow, maxVisibleCol);
}

// used by ResetScrollBars() - This gets only fully visible cells
MCCellRange MCGridCtrl::GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation /*=FALSE*/)
{
    CRect rect;
    GetClientRect(rect);
...
MCCellRange MCGridCtrl::GetSelectedCellRange() const
{
    MCCellRange Selection(GetRowCount(), GetColumnCount(), -1,-1);
...
MCCellRange MCGridCtrl::GetCellRange() const
{
    return MCCellRange(0, 0, GetRowCount() - 1, GetColumnCount() - 1);
}

// Resets the selected cell range to the empty set.
void MCGridCtrl::ResetSelectedRange()
{
    m_PrevSelectedCellMap.RemoveAll();
...
int MCGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */)
{
    SCROLLINFO si;
...
BOOL MCGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
    m_idTopLeftCell.row = -1;
...
void MCGridCtrl::EnableScrollBars(int nBar, BOOL bEnable /*=TRUE*/)
{
    if (bEnable)
    {
...
void MCGridCtrl::ResetScrollBars()
{
    // Force a refresh. 
    m_idTopLeftCell.row = -1;
...
BOOL MCGridCtrl::GetCellOrigin(int nRow, int nCol, LPPOINT p)
{
    int i;
...
BOOL MCGridCtrl::GetCellOrigin(const MCCellID& cell, LPPOINT p)
{
    return GetCellOrigin(cell.row, cell.col, p);
}

// Returns the bounding box of the cell
BOOL MCGridCtrl::GetCellRect(const MCCellID& cell, LPRECT pRect)
{
    return GetCellRect(cell.row, cell.col, pRect);
}

BOOL MCGridCtrl::GetCellRect(int nRow, int nCol, LPRECT pRect)
{
    CPoint CellOrigin;
...
BOOL MCGridCtrl::GetTextRect(const MCCellID& cell, LPRECT pRect)
{
    return GetTextRect(cell.row, cell.col, pRect);
}

BOOL MCGridCtrl::GetTextRect(int nRow, int nCol, LPRECT pRect)
{
    MCGridCellBase* pCell = GetCell( nRow, nCol);
    if( pCell == NULL)
        return FALSE;
...
BOOL MCGridCtrl::GetCellRangeRect(const MCCellRange& cellRange, LPRECT lpRect)
{
...
LRESULT MCGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
...
LRESULT MCGridCtrl::OnGetFont(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
    //LOGFONT    lf;
...
#ifndef _WIN32_WCE_NO_CURSOR
BOOL MCGridCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
    if (nHitTest == HTCLIENT)
    {
        ...
        default:
            if (!GetVirtualMode())
            {
                ...
                ScreenToClient(&pt);
                MCCellID cell = GetCellFromPt(pt);
                if (IsValid(cell))
                {
                    MCGridCellBase* pCell = GetCell(cell.row, cell.col);
                    if (pCell)
...
BOOL MCGridCtrl::SetFixedRowCount(int nFixedRows)
{
...
BOOL MCGridCtrl::SetFixedColumnCount(int nFixedCols)
{
...
BOOL MCGridCtrl::SetRowCount(int nRows)
{
    BOOL bResult = TRUE;
    ...
    if (addedRows < 0)
    {
        if (!GetVirtualMode())
        {
            for (int row = nRows; row < m_nRows; row++)
            {
                ...
                // Delete rows
                MG_GRID_ROW* pRow = m_RowData[row];
                if (pRow)
                    delete pRow;
            }
        }
...
BOOL MCGridCtrl::SetColumnCount(int nCols)
{
    BOOL bResult = TRUE;
...
int MCGridCtrl::InsertColumn(LPCTSTR strHeading,
...
int MCGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    TRY
    {
        ...
        if (nRow < 0)
        {
            ...
            if (!GetVirtualMode())
                m_RowData.Add(new MG_GRID_ROW);
        }
        else
        {
            m_arRowHeights.InsertAt(nRow, (int)0,1);
            if (!GetVirtualMode())
                m_RowData.InsertAt(nRow, new MG_GRID_ROW);
        }
    ...
    if (!GetVirtualMode())
    {
        for (int col = 0; col < m_nCols; col++)
        {
            MG_GRID_ROW* pRow = m_RowData[nRow];
            if (!pRow)
                return -1;
...
BOOL MCGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
    if (GetVirtualMode())
        return FALSE;
    ...
    if (!pRuntimeClass->IsDerivedFrom(RUNTIME_CLASS(MCGridCellBase)))
    {
        ASSERT( FALSE);
        return FALSE;
    }

    MCGridCellBase* pNewCell = (MCGridCellBase*) pRuntimeClass->CreateObject();

    MCGridCellBase* pCurrCell = GetCell(nRow, nCol);
    if (pCurrCell)
        *pNewCell = *pCurrCell;
...
BOOL MCGridCtrl::SetDefaultCellType( CRuntimeClass* pRuntimeClass)
{
...
/*virtual*/ MCGridCellBase* MCGridCtrl::CreateCell(int nRow, int nCol)
{
    ASSERT(!GetVirtualMode());

    if (!m_pRtcDefault || !m_pRtcDefault->IsDerivedFrom(RUNTIME_CLASS(MCGridCellBase)))
    {
        ASSERT( FALSE);
        return NULL;
    }
    MCGridCellBase* pCell = (MCGridCellBase*) m_pRtcDefault->CreateObject();
    if (!pCell)
        return NULL;
...
/*virtual*/ void MCGridCtrl::DestroyCell(int nRow, int nCol)
{
...
BOOL MCGridCtrl::DeleteColumn(int nColumn)
{
    if (nColumn < 0 || nColumn >= GetColumnCount())
        return FALSE;
...
BOOL MCGridCtrl::DeleteRow(int nRow)
{
    if (nRow < 0 || nRow >= GetRowCount())
        return FALSE;
...
BOOL MCGridCtrl::DeleteNonFixedRows()
{
    ResetSelectedRange();
...
BOOL MCGridCtrl::DeleteAllItems()
{
    ResetSelectedRange();
...
/////////////////////////////////////////////////////////////////////////////
// MCGridCtrl data functions

// Set CListCtrl::GetNextItem for details
MCCellID MCGridCtrl::GetNextItem(MCCellID& cell, int nFlags) const
{
...
BOOL MCGridCtrl::SortTextItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
    SetSortColumn(nCol);
    SetSortAscending(bAscending);
    ResetSelectedRange();
    SetFocusCell(-1, - 1);
    return MCGridCtrl::SortItems(pfnCellTextCompare, nCol, bAscending, data);
}

void MCGridCtrl::SetCompareFunction(PFNLVCOMPARE pfnCompare)
{
...
// function
BOOL MCGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
    SetSortColumn(nCol);
    SetSortAscending(bAscending);
    ResetSelectedRange();
  ...
  {
    // If header has numeric sort format
    if(GetItemFormat(0,nCol) & DT_SORT_NUMERIC)
    {
      return MCGridCtrl::SortItems(pfnCellNumericCompare, nCol, bAscending, data);
    }
    // Else default text sort
		return MCGridCtrl::SortItems(pfnCellTextCompare, nCol, bAscending, data);
  }
	else
  {
    return MCGridCtrl::SortItems(m_pfnCompare, nCol, bAscending, data);
  }
}

// Sorts on a given column using the supplied compare function (see CListCtrl::SortItems)
BOOL MCGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending,
                          LPARAM data /* = 0 */)
{
    SetSortColumn(nCol);
    SetSortAscending(bAscending);
...
int CALLBACK MCGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);

	MCGridCellBase* pCell1 = (MCGridCellBase*) lParam1;
	MCGridCellBase* pCell2 = (MCGridCellBase*) lParam2;
	if (!pCell1 || !pCell2) return 0;

	return _tcscmp(pCell1->GetText(), pCell2->GetText());
}

int CALLBACK MCGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
...
BOOL MCGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
                          int low, int high)
{
    ...
    while (lo <= hi)
    {
        ...
        if (lo <= hi)
        {
            ...
            if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol), data) != 0)
            {
				for (int col = 0; col < GetColumnCount(); col++)
                {
					MCGridCellBase *pCell = GetCell(lo, col);
...
BOOL MCGridCtrl::SetItem(const MC_ITEM* pItem)
{
    if (!pItem || GetVirtualMode())
        return FALSE;

    MCGridCellBase* pCell = GetCell(pItem->row, pItem->col);
    if (!pCell)
        return FALSE;
    ...
    if( pItem->mask & GVIF_SQLTYPE)
        pCell->SetSQLType(pItem->sqlType);    
    return TRUE;
}

void
MCGridCtrl::InsertItem(int p_row,int p_col,CString& p_text,UINT p_format /* = 0 */)
{
  MC_ITEM item;
...
BOOL MCGridCtrl::GetItem(MC_ITEM* pItem)
{
    if (!pItem)
        return FALSE;
    MCGridCellBase* pCell = GetCell(pItem->row, pItem->col);
    ...
    if( pItem->mask & GVIF_SQLTYPE)
        pItem->sqlType = pCell->GetSQLType();

    return TRUE;
}

BOOL MCGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str)
{
    if (GetVirtualMode())
        return FALSE;
...
BOOL MCGridCtrl::SetItemTextFmt(int nRow, int nCol, LPCTSTR szFmt, ...)
{
    if (GetVirtualMode())
        return FALSE;
...
BOOL MCGridCtrl::SetItemTextFmtID(int nRow, int nCol, UINT nID, ...)
{
    if (GetVirtualMode())
        return FALSE;
...
BOOL MCGridCtrl::SetItemData(int nRow, int nCol, LPARAM lParam)
{
    if (GetVirtualMode())
        return FALSE;
...
LPARAM MCGridCtrl::GetItemData(int nRow, int nCol) const
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    if (!pCell)
        return (LPARAM) 0;
...
BOOL MCGridCtrl::SetItemImage(int nRow, int nCol, int iImage)
{
    if (GetVirtualMode())
        return FALSE;
...
int MCGridCtrl::GetItemImage(int nRow, int nCol) const
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
BOOL MCGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
    ...
    if (bSelected && !(state & GVIS_SELECTED))
    {
        ...
        if (m_SelectedCellMap.Lookup(key, (MCCellID&)cell))
...
UINT MCGridCtrl::GetItemState(int nRow, int nCol) const
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
BOOL MCGridCtrl::SetItemFormat(int nRow, int nCol, UINT nFormat)
{
    if (GetVirtualMode())
        return FALSE;
...
UINT MCGridCtrl::GetItemFormat(int nRow, int nCol) const
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
BOOL MCGridCtrl::SetItemBkColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
    if (GetVirtualMode())
        return FALSE;
...
COLORREF MCGridCtrl::GetItemBkColour(int nRow, int nCol) const
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
BOOL MCGridCtrl::SetItemFgColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
    if (GetVirtualMode())
        return FALSE;
...
COLORREF MCGridCtrl::GetItemFgColour(int nRow, int nCol) const
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
BOOL MCGridCtrl::SetItemFont(int nRow, int nCol, const LOGFONT* plf)
{
    if (GetVirtualMode())
        return FALSE;
...
const LOGFONT* MCGridCtrl::GetItemFont(int nRow, int nCol)
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell) 
...
BOOL MCGridCtrl::IsItemEditing(int nRow, int nCol)
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
long MCGridCtrl::GetVirtualWidth() const
{
...
long MCGridCtrl::GetVirtualHeight() const
{
...
int MCGridCtrl::GetRowHeight(int nRow) const
{
...
int MCGridCtrl::GetColumnWidth(int nCol) const
{
...
BOOL MCGridCtrl::SetRowHeight(int nRow, int height)
{
...
BOOL MCGridCtrl::SetColumnWidth(int nCol, int width)
{
...
int MCGridCtrl::GetFixedRowHeight() const
{
    int nHeight = 0;
...
int MCGridCtrl::GetFixedColumnWidth() const
{
    int nWidth = 0;
...
BOOL MCGridCtrl::AutoSizeColumn(int nCol, UINT nAutoSizeStyle /*=GVS_DEFAULT*/, 
                               BOOL bResetScroll /*=TRUE*/)
{
    ...
    if (GetVirtualMode())
        SendCacheHintToParent(MCCellRange(nStartRow, nCol, nEndRow, nCol));
...
BOOL MCGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
    ASSERT(nRow >= 0 && nRow < m_nRows);
    if (nRow < 0 || nRow >= m_nRows)
        return FALSE;
    ...
    if (GetVirtualMode())
        SendCacheHintToParent(MCCellRange(nRow, 0, nRow, nNumColumns));
...
void MCGridCtrl::AutoSizeColumns(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    int nNumColumns = GetColumnCount();
    for (int nCol = 0; nCol < nNumColumns; nCol++)
    {
...
void MCGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    CDC* pDC = GetDC();
    if (!pDC)
        return;
    ...
    if (GetVirtualMode())
        SendCacheHintToParent(MCCellRange(nStartRow, 0, nEndRow, nNumColumns));
...
void MCGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    if (bExpandFixed)
	{
...
void MCGridCtrl::ExpandLastColumn()
{
    if (GetColumnCount() <= 0)
        return;
...
void MCGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    if (bExpandFixed)
	{
...
void MCGridCtrl::ExpandToFit(BOOL bExpandFixed /*=TRUE*/)
{
...
void MCGridCtrl::SetVirtualMode(BOOL bVirtual)
{
    DeleteAllItems();
...
void MCGridCtrl::SetGridLines(int nWhichLines /*=GVL_BOTH*/) 
{
    m_nGridLines = nWhichLines;
    Refresh();
}

void MCGridCtrl::SetListMode(BOOL bEnableListMode /*=TRUE*/)
{
    ResetSelectedRange();
    SetSortColumn(-1);
...
void MCGridCtrl::SetSortColumn(int nCol)
{
    if (m_nSortColumn >= 0)
...
BOOL MCGridCtrl::IsCellFixed(int nRow, int nCol)
{
    return (nRow < GetFixedRowCount() || nCol < GetFixedColumnCount());
}

void MCGridCtrl::SetModified(BOOL bModified /*=TRUE*/, int nRow /*=-1*/, int nCol /*=-1*/)
{
...
BOOL MCGridCtrl::GetModified(int nRow /*=-1*/, int nCol /*=-1*/)
{
    if (nRow >= 0 && nCol >= 0)
...
void MCGridCtrl::EnsureVisible(int nRow, int nCol)
{
    ...
    */
    ...
    MCCellRange VisibleCells = GetVisibleNonFixedCellRange();
...
BOOL MCGridCtrl::IsCellEditable(int nRow, int nCol) const
{
    return IsEditable() && ((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY);
}

BOOL MCGridCtrl::IsCellSelected(MCCellID &cell) const
{
    return IsCellSelected(cell.row, cell.col);
}

BOOL MCGridCtrl::IsCellSelected(int nRow, int nCol) const
{
    if (GetVirtualMode())
    {   
        if (!IsSelectable())
        ...
        return (m_SelectedCellMap.Lookup(key, (MCCellID&)cell));       
    }
    else
        return IsSelectable() && ((GetItemState(nRow, nCol) & GVIS_SELECTED) == GVIS_SELECTED);
}

BOOL MCGridCtrl::IsCellVisible(MCCellID cell) 
{
    return IsCellVisible(cell.row, cell.col);
}

BOOL MCGridCtrl::IsCellVisible(int nRow, int nCol)
{
    if (!IsWindow(m_hWnd))
        return FALSE;
...
BOOL MCGridCtrl::InvalidateCellRect(const MCCellID& cell)
{
    return InvalidateCellRect(cell.row, cell.col);
}

BOOL MCGridCtrl::InvalidateCellRect(const int row, const int col)
{
...
BOOL MCGridCtrl::InvalidateCellRect(const MCCellRange& cellRange)
{
    ASSERT(IsValid(cellRange));
    if (!::IsWindow(GetSafeHwnd()) || !m_bAllowDraw)
        return FALSE;

    MCCellRange visibleCellRange = GetVisibleNonFixedCellRange().Intersect(cellRange);
...
void MCGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
    CRect rect;
    GetClientRect(rect);
        ...
        (m_bLMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
            MCGridCellBase* pCell = NULL;
            MCCellID idCurrentCell;
...
CPoint MCGridCtrl::GetPointClicked(int nRow, int nCol, const CPoint& point)
{
...
void MCGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    TRACE0("MCGridCtrl::OnLButtonDblClk\n");

    MCCellID cell = GetCellFromPt(point);
    if( !IsValid( cell) )
    {
        //ASSERT(FALSE);
        return;
...
void MCGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
#ifdef GRIDCONTROL_USE_TITLETIPS
    // EFW - Bug Fix
    m_TitleTip.Hide();  // hide any titletips
    ...
    // TRACE0("MCGridCtrl::OnLButtonDown\n");
    ...
    MCGridCellBase* pCell = GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
    if (pCell)
...
void MCGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    // TRACE0("MCGridCtrl::OnLButtonUp\n");
    ...
    {
        MCGridCellBase* pCell = GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
        if (pCell)
...
#ifndef _WIN32_WCE
void MCGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
...
void MCGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    else
    {
        ...
        // tell the cell about it
        MCGridCellBase* pCell = GetCell(FocusCell.row, FocusCell.col);
        if (pCell)
...
void MCGridCtrl::SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight,
...
void MCGridCtrl::GetPrintMarginInfo(int &nHeaderHeight, int &nFooterHeight,
    int &nLeftMargin, int &nRightMargin, int &nTopMargin,
    int &nBottomMargin, int &nGap)
{
...
void MCGridCtrl::Print(CPrintDialog* pPrntDialog /*=NULL*/)
{
    CDC dc;
...
/////////////////////////////////////////////////////////////////////////////
// MCGridCtrl printing overridables - for Doc/View print/print preview framework

// EFW - Various changes in the next few functions to support the
// new print margins and a few other adjustments.
void MCGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
...
void MCGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
    if (!pDC || !pInfo)
        return;
    ...
    while (m_nCurrPrintRow < GetRowCount())
    {
        ...
        for (int col = m_nPrintColumn; col < GetColumnCount(); col++)
        {
            ...
            MCGridCellBase* pCell = GetCell(m_nCurrPrintRow, col);
            if (pCell)
...
void MCGridCtrl::PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
                                   CDC *pDC, BOOL& bFirst)
{
   ...
   for (int col =nStartColumn; col < nStopColumn; col++)
   {
      ...
      MCGridCellBase* pCell = GetCell(row, col);
      if (pCell)
...
} // end of MCGridCtrl::PrintFixedRowCells

void MCGridCtrl::PrintColumnHeadings(CDC *pDC, CPrintInfo* /*pInfo*/)
{
    CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
    ...
    pDC->SelectObject(pOldFont);
} // end of MCGridCtrl::PrintColumnHeadings
...
*****************************************************************************/
void MCGridCtrl::PrintRowButtons(CDC *pDC, CPrintInfo* /*pInfo*/)
{
    CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
    ...
    for( int iCol = 0; iCol < GetFixedColumnCount(); iCol++)
    {
        ...
        for( int iRow = m_nCurrPrintRow; iRow < GetRowCount(); iRow++)
        {
            ...
            MCGridCellBase* pCell = GetCell(iRow, iCol);
            if (pCell)
...
void MCGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
...
void MCGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
...
void MCGridCtrl::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
...
#ifndef _WIN32_WCE
/////////////////////////////////////////////////////////////////////////////
// MCGridCtrl persistance

BOOL MCGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    CStdioFile File;
    CFileException ex;
...
BOOL MCGridCtrl::Load(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    if (GetVirtualMode())
        return FALSE;
...
/////////////////////////////////////////////////////////////////////////////
// MCGridCtrl overrideables

#ifndef GRIDCONTROL_NO_DRAGDROP
// This is no longer needed since I've changed to OLE drag and drop - but it's
// still cool code. :)
CImageList* MCGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
    CDC* pDC = GetDC();
    if (!pDC)
        return NULL;
...
void MCGridCtrl::OnFixedRowClick(MCCellID& cell)
{
    if (!IsValid(cell))
        return;
...
void MCGridCtrl::OnFixedColumnClick(MCCellID& cell)
{
    if (!IsValid(cell))
        return;
...
CSize MCGridCtrl::GetTextExtent(int nRow, int nCol, LPCTSTR str)
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    if (!pCell)
        return CSize(0, 0);
...
// virtual
void MCGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
#ifndef GRIDCONTROL_NO_TITLETIPS
    m_TitleTip.Hide();  // hide any titletips
#endif
...
// virtual
void MCGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
    CString strCurrentText = GetItemText(nRow, nCol);
...
// virtual
BOOL MCGridCtrl::ValidateEdit(int nRow, int nCol, LPCTSTR str)
{
    MCGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
// virtual
CString MCGridCtrl::GetItemText(int nRow, int nCol) const
{
