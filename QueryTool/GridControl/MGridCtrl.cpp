...
void MCGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
  ...
  InvalidateCellRect(MCCellID(pgvItem->row,pgvItem->col));
  *pResult = 0;
}
...
MCCellID MCGridCtrl::SetFocusCell(MCCellID cell)
{
  ...
  // Force focus to be in a non-fixed cell
                                 ...
                                 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
  ...
  // Don't allow selection of fixed rows
...
MCCellRange MCGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/,BOOL bForceRecalculation /*=FALSE*/)
{
  CRect rect;
  GetClientRect(rect);
...
BOOL MCGridCtrl::SetItem(const MC_ITEM* pItem)
{
  ...
  if( pItem->mask & GVIF_SQLTYPE)      pCell->SetSQLType(pItem->sqlType);    
  return TRUE;
}
...
BOOL MCGridCtrl::GetItem(MC_ITEM* pItem)
{
  ...
  if( pItem->mask & GVIF_SQLTYPE)    pItem->sqlType = pCell->GetSQLType();
...
BOOL MCGridCtrl::AutoSizeColumn(int nCol, UINT nAutoSizeStyle /*=GVS_DEFAULT*/,BOOL bResetScroll /*=TRUE*/)
{
  ASSERT(nCol >= 0 && nCol < m_nCols);
  if (nCol < 0 || nCol >= m_nCols)
  {
...
void MCGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
  ...
  {
    SetFocusCell(-1,-1);
    SetFocusCell(max(FocusCell.row, m_nFixedRows),max(FocusCell.col, m_nFixedCols));
...
void MCGridCtrl::PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,CDC *pDC, BOOL& bFirst)
{
