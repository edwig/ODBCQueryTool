...
typedef struct tagMG_NM_GRIDVIEW 
{
    NMHDR hdr;
    int   iRow;
    int   iColumn;
...
typedef struct tagMG_GV_DISPINFO 
{
    NMHDR   hdr;
    MC_ITEM item;
}
...
// This is sent to the Grid from child in-place edit controls
typedef struct tagMG_GV_CACHEHINT 
{
    NMHDR      hdr;
    MCCellRange range;
}
...
typedef CTypedPtrArray<CObArray, MCGridCellBase*> MG_GRID_ROW;

// For virtual mode callback
typedef BOOL (CALLBACK* MG_GRIDCALLBACK)(MG_GV_DISPINFO *, LPARAM);
...
class MCGridCtrl : public CWnd
{
    DECLARE_DYNCREATE(MCGridCtrl)
    friend class MCGridCell;
    friend class MCGridCellBase;

// Construction
public:
    MCGridCtrl(int nRows = 0, int nCols = 0, int nFixedRows = 0, int nFixedCols = 0);

    BOOL Create(const RECT& rect, CWnd* parent, UINT nID,
                DWORD dwStyle = WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE);

    void RegisterView(CGridView* p_view);
///////////////////////////////////////////////////////////////////////////////////
// Attributes
///////////////////////////////////////////////////////////////////////////////////
public:
    ...
    BOOL GetCellOrigin(const MCCellID& cell, LPPOINT p);
    BOOL GetCellRect(int nRow, int nCol, LPRECT pRect);
    BOOL GetCellRect(const MCCellID& cell, LPRECT pRect);

    BOOL GetTextRect(const MCCellID& cell, LPRECT pRect);
    BOOL GetTextRect(int nRow, int nCol, LPRECT pRect);

    MCCellID GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck = TRUE);
    ...
    MCCellID SetFocusCell(MCCellID cell);
    MCCellID SetFocusCell(int nRow, int nCol);
    MCCellID GetFocusCell() const                  { return m_idCurrentCell;           }


    void SetVirtualMode(BOOL bVirtual);
    BOOL GetVirtualMode() const                   { return m_bVirtualMode;            }
    void SetCallbackFunc(MG_GRIDCALLBACK pCallback, 
                         LPARAM lParam)           { m_pfnCallback = pCallback; m_lParam = lParam; }
    MG_GRIDCALLBACK GetCallbackFunc()                { return m_pfnCallback;             }
...
///////////////////////////////////////////////////////////////////////////////////
public:
    MCGridCellBase* GetDefaultCell(BOOL bFixedRow, BOOL bFixedCol) const;

///////////////////////////////////////////////////////////////////////////////////
// Grid cell Attributes
///////////////////////////////////////////////////////////////////////////////////
public:
    MCGridCellBase* GetCell(int nRow, int nCol) const;   // Get the actual cell!
    ...
    BOOL   SetItem(const MC_ITEM* pItem);
    BOOL   GetItem(MC_ITEM* pItem);
...
public:
	...
	void ClearCells(MCCellRange Selection);
    ...
    BOOL IsCellEditable(MCCellID &cell) const;
    BOOL IsCellSelected(int nRow, int nCol) const;
    BOOL IsCellSelected(MCCellID &cell) const;
    ...
    BOOL RedrawCell(const MCCellID& cell, CDC* pDC = NULL);
...
///////////////////////////////////////////////////////////////////////////////////
 public:
    MCCellRange GetCellRange() const;
    MCCellRange GetSelectedCellRange() const;
    void SetSelectedRange(const MCCellRange& Range, BOOL bForceRepaint = FALSE, BOOL bSelectCells = TRUE);
    void SetSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
                          BOOL bForceRepaint = FALSE, BOOL bSelectCells = TRUE);
    BOOL IsValid(int nRow, int nCol) const;
    BOOL IsValid(const MCCellID& cell) const;
    BOOL IsValid(const MCCellRange& range) const;
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    virtual BOOL PasteTextToGrid(MCCellID cell, COleDataObject* pDataObject, BOOL bSelectPastedCells=TRUE);
#endif
...
///////////////////////////////////////////////////////////////////////////////////
public:
    MCCellID GetNextItem(MCCellID& cell, int nFlags) const;
...
protected:
    ...
    LRESULT SendDisplayRequestToParent(MG_GV_DISPINFO* pDisplayInfo) const;
    LRESULT SendCacheHintToParent(const MCCellRange& range) const;

    BOOL InvalidateCellRect(const int row, const int col);
    BOOL InvalidateCellRect(const MCCellID& cell);
    BOOL InvalidateCellRect(const MCCellRange& cellRange);
    void EraseBkgnd(CDC* pDC);

    BOOL GetCellRangeRect(const MCCellRange& cellRange, LPRECT lpRect);

    BOOL SetCell(int nRow, int nCol, MCGridCellBase* pCell);
    ...
    MCCellID GetTopleftNonFixedCell(BOOL bForceRecalculation = FALSE);
    MCCellRange GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation = FALSE);
    MCCellRange GetVisibleNonFixedCellRange(LPRECT pRect = NULL, BOOL bForceRecalculation = FALSE);
...
protected:
    ...
    // Mouse Clicks
    virtual void  OnFixedColumnClick(MCCellID& cell);
    virtual void  OnFixedRowClick(MCCellID& cell);
    ...
    virtual MCGridCellBase* CreateCell(int nRow, int nCol);
...
protected:
    ...
    MG_GRIDCALLBACK m_pfnCallback;                                     // The callback function
    ...
    MCGridDefaultCell m_cellDefault;  // "default" cell. Contains default colours, font etc.
    MCGridDefaultCell m_cellFixedColDef, m_cellFixedRowDef, m_cellFixedRowColDef;
    CFont       m_PrinterFont;  // for the printer
    CImageList* m_pImageList;

    // Cell data
    CTypedPtrArray<CObArray, MG_GRID_ROW*> m_RowData;

    // Mouse operations such as cell selection
    int         m_MouseMode;
    BOOL        m_bLMouseButtonDown, m_bRMouseButtonDown;
    CPoint      m_LeftClickDownPoint, m_LastMousePoint;
    MCCellID     m_LeftClickDownCell, m_SelectionStartCell;
    MCCellID     m_idCurrentCell, m_idTopLeftCell;
    UINT_PTR    m_nTimerID;
    int         m_nTimerInterval;
    int         m_nResizeCaptureRange;
    BOOL        m_bAllowRowResize, m_bAllowColumnResize;
    int         m_nRowsPerWheelNotch;
    CMap<DWORD,DWORD, MCCellID, MCCellID&> m_SelectedCellMap, m_PrevSelectedCellMap;

    // Query view - used for delayed reads
    CGridView*  m_view;
#ifndef GRIDCONTROL_NO_TITLETIPS
    ...
    // Drag and drop
    MCCellID     m_LastDragOverCell;
#ifndef GRIDCONTROL_NO_DRAGDROP
    MCGridDropTarget m_DropTarget;       // OLE Drop target for the grid
#endif
...
protected:
    void SelectAllCells();
    void SelectColumns(MCCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void SelectRows(MCCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void SelectCells(MCCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void OnSelecting(const MCCellID& currentCell);
...
inline MCGridCellBase* MCGridCtrl::GetDefaultCell(BOOL bFixedRow, BOOL bFixedCol) const
{ 
    if (bFixedRow && bFixedCol) return (MCGridCellBase*) &m_cellFixedRowColDef;
    if (bFixedRow)              return (MCGridCellBase*) &m_cellFixedRowDef;
    if (bFixedCol)              return (MCGridCellBase*) &m_cellFixedColDef;
    return (MCGridCellBase*) &m_cellDefault;
}

inline MCGridCellBase* MCGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        MCGridCellBase* pCell = GetDefaultCell(nRow < m_nFixedRows, nCol < m_nFixedCols);
        static MG_GV_DISPINFO gvdi;
        gvdi.item.row     = nRow;
        gvdi.item.col     = nCol;
        ...
        if (GetFocusCell() == MCCellID(nRow, nCol)) gvdi.item.nState |= GVIS_FOCUSED;
        ...
        cell.SetGrid((MCGridCtrl*)this);

        return (MCGridCellBase*) &cell;
    }
...
inline BOOL MCGridCtrl::SetCell(int nRow, int nCol, MCGridCellBase* pCell)
{
    if (GetVirtualMode())
        return FALSE;
...
inline void 
MCGridCtrl::RegisterView(CGridView* p_view)
{
  m_view = p_view;
}
