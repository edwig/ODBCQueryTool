...
BOOL ZTitleTip::Create(CWnd * pParentWnd)
{
...
void ZTitleTip::Show(CRect rectTitle, LPCTSTR lpszTitleText, int xoffset /*=0*/,
...
void ZTitleTip::OnMouseMove(UINT nFlags, CPoint point) 
{
...
BOOL ZTitleTip::PreTranslateMessage(MSG* pMsg) 
{
  ...
  BOOL bDoubleClick=FALSE;
	...
	switch (pMsg->message)
	{
	  case WM_LBUTTONDOWN:    // Get tick count since last LButtonDown
                            dwTick = GetTickCount();
	  ...
	  case WM_RBUTTONDOWN:
	  case WM_MBUTTONDOWN: {  POINTS pts = MAKEPOINTS( pMsg->lParam );
		                        POINT  point;
		                        point.x = pts.x;
		                        point.y = pts.y;
