    ...
    struct COEditorViewDataSource : COleDataSource
    {
      ...
      virtual BOOL OnRenderGlobalData (LPFORMATETC lpFormatEtc, HGLOBAL* phGlobal)
      {
        ...
        if (lpFormatEtc->cfFormat == format)
        {
          ...
          if (hGlobal)
          {
            LPSTR buff = (LPSTR)GlobalLock(hGlobal);
            if (buff)
            {
              memcpy(buff,text.GetString(),(text.GetLength() + 1) * sizeof(TCHAR));
...
DROPEFFECT COEditorView::OnDragOver (COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
  ...
  if (pDataObject && pDataObject->IsDataAvailable(format))
  {
    ...
    if (m_DragAndDropData.ptCursor == point)  // no cursor motion
    {
      ...
      switch (status)
      {
        case start: SetTimer(DRAG_MOUSE_TIMER, DRAG_MOUSE_TIMER_ELAPSE, NULL);
                    break;
        case redraw:UpdateWindow();
                    m_DragAndDropData.bVisible = false;
...
BOOL COEditorView::OnDrop (COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point) 
{
  ...
  if (pDataObject 
  && pDataObject->IsDataAvailable(format)
  && !((m_DragAndDropData.pSrcWnd == this) && CursorOnSelection(point))) 
  {
    HGLOBAL hGlobal = pDataObject->GetGlobalData(format);
    if (hGlobal) 
    {
      ...
      bool move = (dropEffect == DROPEFFECT_MOVE && m_DragAndDropData.pSrcWnd == this) ? true : false;
            
      bool after = false; // where's the drop position ? after the STREAM selection position? 
      ...
      if (move)
      {
        ...
        switch (GetBlockMode())
        {
          case ebtColumn: {   int inx = -1;
                          ...
                          break;
          case ebtStream: if (pos.line < orgSel.start.line 
      ...
      // insert
      if (LPTSTR buff = (LPTSTR)GlobalLock(hGlobal)) 
      {
