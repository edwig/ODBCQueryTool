...
void 
COEditorView::OnFilePrintPreview()
{
  // must not create this on the frame. Must outlive this function
  CPrintPreviewState* pState = new CPrintPreviewState;

  if(!DoPrintPreview(IDD_AFXBAR_RES_PRINT_PREVIEW, this, RUNTIME_CLASS(CPreviewViewEx), pState))
  {
...
BOOL 
COEditorView::OnPreparePrinting (CPrintInfo* pInfo)
{
	return DoPreparePrinting(pInfo);
}

inline void 
HIMETRICtoLP(CDC* pDC, CPoint& pt)
{
  CSize sz(pt.x, pt.y);
  pDC->HIMETRICtoLP(&sz);
...
//  const long logPixelsY     = pDC->GetDeviceCaps(LOGPIXELSY);
  ...
  // bug fix, wrong print highlighting after print preview
...
void 
COEditorView::OnEndPrinting (CDC*, CPrintInfo* pInfo)
{
...
void 
COEditorView::OnPrepareDC (CDC*, CPrintInfo* pInfo)
{
...
void 
COEditorView::OnPrint (CDC* pDC, CPrintInfo* pInfo)
{
  _ASSERTE(pInfo->m_lpUserData);
    ...
    while (!tokenizer.Eol())
    {
      ...
      if (pos >= pc.m_cacheWordWrapOffset)
      {
        ...
        if (phighlighter)
        {
          ...
          {
            pDC->SetTextColor(phighlighter->GetTextColor());
          }
        }
...
void 
COEditorView::MoveToPage (CPrintInfo* pInfo, int* pMaxPage)
{
    int orgCurPage = -1;
...
void 
COEditorView::PrintHeader (CDC* pDC, CPrintInfo* pInfo)
{
  PrintContext& pc = *(PrintContext*)pInfo->m_lpUserData;
...
void 
COEditorView::PrintFooter (CDC* pDC, CPrintInfo* pInfo)
{
  PrintContext& pc = *(PrintContext*)pInfo->m_lpUserData;
...
void 
COEditorView::GetMargins (CRect& rc) const
{
