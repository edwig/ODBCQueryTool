...
- Repaired the representation of the ODBC datatypes, as reported by the driver
  The following where incorrect and repaired:

  money              // Money (FIXED_PRECISION_SCALE) (TRUE,FALSE)
  autoincrement      // Auto incrementable (TRUE,FALSE,NULL for non-numeric)
  local_type_name    // Local type name for display on UI's (not in DDL!)
  minimum_scale      // Minimum scale of datatype. E.g. in seconds of the TIMESTAMP
  maximum_scale      // Maximum scale of datatype, otherwise use column_size

  The following where added

  sqlDatatype        // Driver independent SQL datatype
  sqlSubType         // SQL subtype for TYPE_TIMESTAMP and INTERVAL types
  radix              // Decimal radix (2,10 or NULL)
  interval_precision // Number of decimals in interval precision of leading type

  After this reparation, the ODBC types info tree (left hand side) is now correct again.
